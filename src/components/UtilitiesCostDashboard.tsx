...
export const UtilitiesCostDashboard: React.FC<UtilitiesCostDashboardProps> = ({ costLines }) => {
  ...
  const utilLine = effectiveLines.find(l => l.name === 'Utilities');
  const UTIL_TOTAL = utilLine ? utilLine.total : 0;
  const maintLine = effectiveLines.find(l => l.name === 'Maintenance');
  const MAINT_TOTAL = maintLine ? maintLine.total : 0;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
          ...
          <div className="text-[10px] text-gray-400 mt-1">{GRAND_TOTAL !== 0 ? ((UTIL_TOTAL / GRAND_TOTAL) * 100).toFixed(1) : '0.0'}% {t('of total')}</div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
          ...
          <div className="text-[10px] text-gray-400 mt-1">{GRAND_TOTAL !== 0 ? ((MAINT_TOTAL / GRAND_TOTAL) * 100).toFixed(1) : '0.0'}% {t('of total')}</div>
        </div>
