...
*   **UI/UX & Navigation**:
    ...
    *   **Landing Page**: 3 cards — Pallacanestro Varese (→ InternalHub), Varese Basketball (→ VBHub), Our Projects (→ ProjectsHub).
    *   **ProjectsHub**: Card picker with 2 project cards: "Training Facility - Campus" (emerald theme) and "Arena Remodelation" (blue theme). Each card opens a detail page with hero image, status badge, project details grid, milestone tracker with progress bar, and a placeholder for future content. Route: `#projects`.
...
*   **Financial & Analytics Features**:
    ...
    *   **5 Year Financial Plan**: Google Sheets-synced long-term financial projections page covering 10 fiscal years (4 historical actuals + 6 projected). Uses `five_year` module key with the standard sheet-config/sync-sheet/sheet-data pattern. Features: sparkline key metric cards (EBITDA, EBIT, EBT, NI) with actual vs projected distinction; stacked revenue breakdown chart with projection divider line; profitability path chart with zero-line reference; cost structure & gross margin dual-axis chart; collapsible P&L/Balance Sheet/Cash Flow tables with sticky label column, current-year highlighting (amber), and muted projected columns. Projection start auto-detected from current fiscal year. Locale-safe number parsing handles both EU (1.234,56) and US (1,234.56) formats, parenthesized negatives. Component: `src/components/FiveYearPlan.tsx`, route: `#five-year`, accessible from 5th card in FinancialCenter.
