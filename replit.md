...
-   **Game Performance Dashboard**: Sources individual game statistics from BigQuery table `ticketing-migration.ticketing_migration.sg_ind_games`. Features three sub-tabs: Team Performance (aggregate stats, scoring charts, roster stats, game log), Player Performance (individual stats, scoring trends, detailed game log), and Stats Search (search by player name, team, or opponent). Data fetched via `/api/vb/ind-games` endpoint with 10-minute server-side cache. Teams and players are derived dynamically from the data. Teams: Varese U19, Campus Varese, Robur et Fides Varese U17, Pallacanestro Varese U17, Robur et Fides U19.
