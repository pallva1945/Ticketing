...
*   **Financial & Analytics Features**:
    ...
    *   **5 Year Financial Plan**: Google Sheets-synced long-term financial projections page. Uses `five_year` module key with the standard sheet-config/sync-sheet/sheet-data pattern. Features stacked bar + line chart, season summary cards with YoY growth, and collapsible section breakdown table. Locale-safe number parsing handles both EU (1.234,56) and US (1,234.56) formats. Component: `src/components/FiveYearPlan.tsx`, route: `#five-year`, accessible from 5th card in FinancialCenter.
